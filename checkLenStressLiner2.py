...
import json
import setup
from algorithm.SGDBase import egraphTorusSGD, egraphSGD
...
def create_graph(graph, file_name, multiple_num, i):
    ...
    sgd_log = egraphSGD.sgd(graph, file_name, i)
...
def get_stress_by_len(graph, file_name):
    ...
    for i in range(1, COUNT+1):
        n = i*0.05
        torus_log, sgd_log= create_graph(graph, file_name, n, i)
        
        data.append([n, torus_log["stress"], sgd_log["stress"]])
        all_log[n] = {"1":{"torusSGD":torus_log, "sgd_log":sgd_log}}
